...
MAIN.CXX = $(filter %.cpp %.cxx,$(wildcard $(srcdir)main.*))
#============
ifneq ($(strip $(filter install install-bin,$(MAKECMDGOALS))),)
...
$(buildir)$(prog_name): $(LIBS) $(buildir)$(INSTALLSTAMP) $(MAIN.CXX)
endif
ifndef SHARED
	$(CXX) $(CXXFLAGS) -o $@ $(INCLUDES) $(MAIN.CXX) $(CXXLIBS)
else
	$(CXX) $(filter-out -pic -fpic -Fpic,$(CXXFLAGS)) -o $@ $(INCLUDES) -Wl,-rpath="$(RPATH)" $(MAIN.CXX) $(CXXLIBS)
endif
